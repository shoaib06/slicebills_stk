...
class Admin extends CI_Controller
{
	...
	public function index()
	{
		...
		$config["total_rows"] = $this->common_model->GetTotalCount(PURPOSES_TABLE, $where);
		...
		$page_data["results"] = $this->common_model->GetTableRows(PURPOSES_TABLE,$where,array('id','desc'),$limit,$page);
	...
	public function add()
	{
		if($this->input->post('s')) // if form submitted
		{
			...
			if ($this->form_validation->run() == true)
			{
				...
				$insert_id = $this->common_model->InsertTableData(PURPOSES_TABLE,$data);
				if($insert_id)
				{
					$this->session->set_flashdata('flash_message','Purpose successfully added.');
	...
	{
		$page_data = (array)$this->common_model->GetSingleRow(PURPOSES_TABLE,array('id' => $id)); // get data
		...
		if($this->input->post('s')) // if form submitted
		{
			...
			if ($this->form_validation->run() == true)
			{
				...
				$update_data = $this->common_model->UpdateTableData(PURPOSES_TABLE,$data,array('id' => $id));
				if($update_data)
				{
					$this->session->set_flashdata('flash_message','Purpose successfully updated.');
					$this->session->set_flashdata('class','success');
					redirect(base_url($this->path));
					die();
				}
	...
	public function delete()
	{
		...
		$update_data = $this->common_model->DeleteTableData(PURPOSES_TABLE,array('id' => $id));
		if($update_data)
		{
			$result['status'] = "success";
			$result['html'] = "Purpose Successfully Deleted.";
			$result['type'] = "alert";
		}
		else
		{
