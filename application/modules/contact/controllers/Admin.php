...
class Admin extends CI_Controller
{
	...
	public function index()
	{
		...
		$config["total_rows"] = $this->common_model->GetTotalCount('tbl_contact', $where);
		...
		$page_data["results"] = $this->common_model->GetTableRows('tbl_contact',$where,array('id','desc'),$limit,$page);
	...
	{ 
	   ...
	   header("Content-Type: application/csv; "); 
		...
		$where = self::filter_query($where);
		$where = implode(' AND ', $where);
		$results = $this->common_model->GetTableRows('tbl_contact',$where,array('id','desc'));
		
	   // file creation 
	   $file = fopen('php://output', 'w');
	 
	   $header = array("Email","Name","Massage","Date");
		fputcsv($file, $header);
	   foreach($results as $key=>$data){

			$new_data['email'] = $data['email'];
			$new_data['name'] = $data['name'];
			$new_data['massage'] = $data['massage'];
