...
class Admin extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // echo $user_id;die;
        $where = [];
        // $where[] = "id <> '$user_id' and user_type=2 and deleted_at<1";
        $where = self::filter_query($where);
        $where = implode(' AND ', $where);

        $config["base_url"] = base_url() . $this->path . "/" . $this->method_name;
        $config["total_rows"] = $this->common_model->GetTotalCount('tbl_donot_sell_requests', $where);
        ...
        $page_data["results"] = $this->common_model->GetTableRows('tbl_donot_sell_requests', $where, ['id', 'desc'], $limit, $page);
    ...
    public function add()
    {
        ...
        if ($this->input->post('s')) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $insert_id = $this->common_model->InsertTableData('tbl_donot_sell_requests', $data);
                if ($insert_id) {
    ...
    {
        // Update function
        $page_data = (array) $this->common_model->GetSingleRow('tbl_donot_sell_requests', ['id' => $id]); // get data
        if (!count($page_data)) {
            redirect(base_url($this->_module . '/admin'));
        }

        $page_data["states"] = $this->common_model->GetTableRows('tbl_states', [], ['id', 'asc']);
        ...
        if ($this->input->post('s')) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $update_data = $this->common_model->UpdateTableData('tbl_donot_sell_requests', $data, ['id' => $id]);
                if ($update_data) {
    ...
    function check_email()
    {
        ...
        $count = $this->common_model->checkExists('tbl_donot_sell_requests', ['email' => $email]);
        if ($count == 0) {
            return true;
        } else {
    ...
    public function change_status()
    {
        ...
        $this->db->update('tbl_donot_sell_requests');
